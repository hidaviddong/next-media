...
import { library, library_movies, movie } from "@next-media/db/schema";
import { and, eq } from "drizzle-orm";
import { db } from "@next-media/db/db";
import { TMDB_BASE_URL } from "@next-media/configs/constant";
    ...
    ffprobe.on("close", (code) => {
      if (code === 0) {
        ...
        try {
          ...
          console.error(e);
          reject(
            new Error("Failed to parse ffprobe JSON output.", { cause: e })
          );
        }
      } else {
        // 如果退出码非 0，代表失败。
